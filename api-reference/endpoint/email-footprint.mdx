...
The Email Footprint API checks whether a specific email address is registered across social media, shopping, and professional platforms.
...
<ResponseField name="stats" type="object">
  ...
  <Expandable title="properties">
    ...
    <ResponseField name="found" type="number">Number of platforms where the email was found.</ResponseField>
...
```bash cURL
# 1. Create the task
curl "https://www.osintcat.net/api/footprint/create-task?query=john@gmail.com&type=email" \
...
task = requests.get(
    ...
    params={"query": "john@gmail.com", "type": "email"},
    headers=HEADERS,
).json()
...
(async () => {
  ...
  const task = await fetch(
    `${BASE}/api/footprint/create-task?query=john@gmail.com&type=email`,
    { headers: HEADERS }
  ).then(r => r.json());
...
// 1. Create the task (charges one lookup)
$task = get_json("$base/api/footprint/create-task?query=john@gmail.com&type=email", $headers);
$taskId = $task["task_id"];
